...
const LectureRow = memo(({ lecture, onAdd }: { lecture: Lecture; onAdd: (lecture: Lecture) => void }) => {
  return (
    <Tr>
...
const fetchMajors = (() => {
  ...
  return () => {
    if (!cache) cache = axios.get<Lecture[]>("/schedules-majors.json");
    return cache;
  };
})();
const fetchLiberalArts = (() => {
  let cache: Promise<AxiosResponse<Lecture[]>> | null = null;
  return () => {
    if (!cache) cache = axios.get<Lecture[]>("/schedules-liberal-arts.json");
    return cache;
  };
})();
